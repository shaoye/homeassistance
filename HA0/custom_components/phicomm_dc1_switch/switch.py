...
#     ip = config.get(CONF_IP)
    ...
    try:
        ...
        # _LOGGER.error("PhicommDC1Switch server got %s,dc1sock is : %s,name:%s" % (e,dc1sock,name))
        time.sleep(0.1)
        pass
    ...
    def pressPlug(self, iport, fOn):
        ...
        try:
            ...
            if fOn is False:
                ...
                # _LOGGER.error('i1:%s', bin(i))
                i |= 0b1
                # _LOGGER.error('i2:%s', bin(i))
                i |= 1 << iport
                # _LOGGER.error('i3:%s', bin(i))
            strT = bin(int(i))
    ...
    def update(self):
        ...
        try:
            ...
            for sockA in self._connection_list:
                if 'closed' in str(sockA):
                    self._connection_list.remove(sockA)
                    # _LOGGER.error("移除关闭端口：%s,self._connection_list:%s" % (sockA,self._connection_list))
                    continue
                else:
                    try:
                        ...
                        if sockA is self.sock:
                            ...
                            _LOGGER.error("PhicommDC1Switch find wrong sockA %s" , sockA)
                        ...
                        # _LOGGER.error(
                            # '%s--334Exception:%s sock:%s,self._connection_list:%s' % (self._name,e, sockA,self._connection_list))
                        # self._connection_list.remove(sockA)
                        #sockA.shutdown(2)
                        sockA.close()
                        continue
            ...
            try:
                ...
                # _LOGGER.error('%s--352Exception:%s,self._connection_list:%s' % (self._name,e,self._connection_list))
                return None
                    ...
                    if sock is self.sock:
                        ...
                        try:
                            ...
                            try:
                                ...
                                # _LOGGER.error('%s--387Exception:%s' % (self._name,e))
                                pass
                                sock.shutdown(2)
                                sock.close()
